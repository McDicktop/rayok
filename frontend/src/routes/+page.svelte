...
<div class="min-h-screen">
    ...
    <header class="sticky top-0 z-[100] border-b border-[#a08c5b]/20 bg-[rgba(17,16,9,0.92)] backdrop-blur-[12px]">
        <div class="max-w-[1200px] mx-auto px-8 flex items-center justify-between h-16">
            <div class="flex items-center gap-[10px]">
                <span class="text-[#c9a84c] text-xl">✦</span>
                <span class="font-['Playfair_Display'] text-xl tracking-[0.05em] text-[#e8e0d0]">Студия «Раёк»</span>
            </div>
            <nav class="flex gap-8 items-center">
                <a href="/" class="text-[0.85rem] tracking-widest uppercase text-[#c9a84c] no-underline transition-colors">
                    Мероприятия
                </a>
                <a href="/admin" class="text-[0.85rem] tracking-widest uppercase text-[#c9a84c] border border-[#c9a84c] px-4 py-[6px] rounded-[2px] no-underline transition-all hover:bg-[#c9a84c] hover:text-[#111009]">
                    Админ панель
                </a>
            </nav>
        </div>
    ...
    <main class="max-w-[1200px] mx-auto px-8 pb-16">
        ...
        <section class="pt-16 pb-10">
            <p class="text-[0.75rem] tracking-[0.2em] uppercase text-[#c9a84c] mb-3">Афиша студии</p>
            <h1 class="font-['Playfair_Display'] text-[clamp(2.5rem,5vw,4rem)] font-bold text-[#f0e8d5] leading-[1.1]">
                Мероприятия
            </h1>
            <div class="w-[60px] h-[2px] bg-gradient-to-r from-[#c9a84c] to-transparent mt-6"></div>
        </section>
        ...
        <div class="flex flex-wrap gap-2 mb-10">
            {#if categories.length > 0}
                <button
                    class="font-['Jost'] text-[0.8rem] tracking-[0.08em] uppercase border rounded-[2px] px-4 py-[6px] cursor-pointer transition-all {selectedCategory === 'all' ? 'bg-[#c9a84c] border-[#c9a84c] text-[#111009]' : 'border-[#a08c5b]/30 bg-transparent text-[#9a9080] hover:border-[#c9a84c] hover:text-[#c9a84c]'}"
                    on:click={() => (selectedCategory = "all")}
                >Все</button>
                {#each categories as cat}
                    <button
                        class="font-['Jost'] text-[0.8rem] tracking-[0.08em] uppercase border rounded-[2px] px-4 py-[6px] cursor-pointer transition-all {selectedCategory === cat._id ? 'bg-[#c9a84c] border-[#c9a84c] text-[#111009]' : 'border-[#a08c5b]/30 bg-transparent text-[#9a9080] hover:border-[#c9a84c] hover:text-[#c9a84c]'}"
        ...
        {:else if filtered.length === 0}
            <div class="flex items-center justify-center py-20">
                <p class="font-['Playfair_Display'] italic text-xl text-[#5a5040]">Мероприятий пока нет</p>
            </div>
        {:else}
            <div class="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-5">
                {#each filtered as event, i}
                    ...
                    >
                        <!-- Image -->
                        <div class="relative aspect-video overflow-hidden bg-[#222018]">
                            {#if event.media?.cover}
                                ...
                                />
                            {:else}
                                <div class="w-full h-full flex items-center justify-center text-[rgba(201,168,76,0.2)] text-3xl">
                                    ✦
                                </div>
                            {/if}
                            <span
                        ...
                        <div class="p-6">
                            <div class="h-[270px]">
                                {#if categories}
                                    <p class="text-[0.7rem] tracking-[0.15em] uppercase text-[#c9a84c] mb-2">
                                        {categories.find((cat) => cat._id === event.category)?.label}
                                    </p>
                                {/if}
                                <h2 class="font-['Playfair_Display'] text-xl font-bold text-[#f0e8d5] leading-[1.3] mb-3">
                                    {event.title}
                                </h2>
                                <p class="text-[0.88rem] leading-relaxed text-[#7a7060] mb-5">
                                ...
                                <div class="flex flex-col gap-[0.35rem] mb-5">
                                    {#if event.schedule?.dates?.length}
                                        <div class="flex items-center gap-2 text-[0.82rem] text-[#9a9080]">
                                            <span class="text-[#c9a84c] text-[0.9rem]">◷</span>
                                            {formatDate(event.schedule.dates[0])}
                                            {#if event.schedule.isSubscription}
                                                <span class="text-[0.65rem] tracking-widest uppercase border border-[#c9a84c] text-[#c9a84c] px-[6px] py-[1px] rounded-[2px] ml-1">
                                                    Абонемент
                                                </span>
                                            {/if}
                                        </div>
                                    ...
                                    {#if event.duration}
                                        <div class="flex items-center gap-2 text-[0.82rem] text-[#9a9080]">
                                            <span class="text-[#c9a84c] text-[0.9rem]">⏱</span>
                                            {#if event.duration.isApproximate}<span class="text-[#c9a84c] ml-[2px]">~</span>{/if}
                                    ...
                                    {#if event.restrictions?.ageMin !== undefined}
                                        <div class="flex items-center gap-2 text-[0.82rem] text-[#9a9080]">
                                            <span class="text-[#c9a84c] text-[0.9rem]">◉</span>
                                            {event.restrictions.ageMin === 0 && event.restrictions.ageMax === 100
                                                ? "Без возрастных ограничений"
                                                : event.restrictions.ageMin + " – " + event.restrictions.ageMax + " лет"}
                                        </div>
                                    {/if}
                                </div>
                            </div>
                            ...
                            <!-- Footer -->
                            <div class="flex items-center justify-between pt-4 border-t border-[#a08c5b]/15">
                                <span class="font-['Playfair_Display'] text-[1.1rem] text-[#c9a84c]">
