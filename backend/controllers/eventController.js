...
const extractS3Key = (url) => {
	if (!url) return null;
	const parts = new URL(url).pathname.split("/").slice(2).join("/");
	return parts || null;
};
...
class EventController {
	async postEvent(req, res) {
		try {
			...
			// 2. JSON fields
			const { data: parsed, error: parseError } = parseJsonFields(req.body);
			if (parseError) {
				return res
					.status(parseError.status)
					.json({ message: parseError.message, error: parseError.code });
			}
			...
			const simpleChecks = [
				...
				[validateRestrictions(newEvent.restrictions), "INVALID_RESTRICTIONS"],
			...
			if (mediaError) {
				return res
					...
					.json({ message: mediaError.message, error: mediaError.code });
			}
	...
	async deleteFiles(req, res) {
		try {
			...
			await Promise.all(keys.map((key) => deleteFromS3(extractS3Key(key))));

			return res.status(200).json({ message: "Files deleted", deleted: keys });
		} catch (e) {
			console.error(e);
			return res.status(500).json({
				message: "Internal server error",
	...
	async deleteEvent(req, res) {
		try {
			...
			if (!event) {
				return res
					.status(404)
					.json({ message: "Event not found", error: "EVENT_MISSING" });
			}
	...
	async updateEvent(req, res) {
		try {
			...
			for (const field of ["title", "description", "cancellationPolicy"]) {
			...
			const allChecks = [
				changes.organizer
					? [validateOrganizer(changes.organizer), "INVALID_ORGANIZER"]
					: null,
				changes.restrictions
					? [validateRestrictions(changes.restrictions), "INVALID_RESTRICTIONS"]
					: null,
			...
			if (req.files?.cover?.[0]) {
				...
				changes["media.cover"] = await uploadToS3(req.files.cover[0], "cover");
			}
			if (req.files?.gallery?.length) {
				...
				changes["media.gallery"] = await Promise.all(
					req.files.gallery.map((file) => uploadToS3(file, "gallery")),
				);
			}
	...
	async deleteCategory(req, res) {
		try {
			...
			if (!isValidId(id)) {
				return res
					.status(400)
					.json({ message: "Invalid category id", error: "INVALID_ID" });
			}

			const hasEvents = await Event.exists({ category: id });
			if (hasEvents) {
				return res.status(409).json({
					message: "Category contains events and therefore cannot be deleted",
	...
	async changeCategory(req, res) {
		try {
			...
			if (!updated) {
				return res
					.status(404)
					.json({ message: "Category not found", error: "CATEGORY_MISSING" });
			}
	...
	async changeEventStatus(req, res) {
		try {
			...
			if (!statuses.includes(status)) {
				return res
					.status(400)
					.json({ message: "Invalid status value", error: "INVALID_STATUS" });
			}
			...
			return res.status(200).json({ message: `Status has been set on '${status}'` })
...
<form action="/upload" method="post" encType="multipart/form-data">
  ...
  await fetch("/upload", {
	method: "POST",
	body: formData,
	// headers НЕ указывать — браузер сам выставит multipart/form-data с boundary
  });
  }
  </script>
...
<form on:submit={handleSubmit}>
  ...
  type="file"
  accept="image/*"
  on:change={e => cover = e.target.files[0]}
  />
  ...
  accept="image/*"
  multiple
  on:change={e => gallery = Array.from(e.target.files)}
  />
